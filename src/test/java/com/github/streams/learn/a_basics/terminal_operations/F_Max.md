...
`max` is the mirror of `min`: it returns the largest element for the given `Comparator`, again as an
`Optional`.

```java
int oldestAge = explorers.stream()
        .max(Comparator.comparing(Explorer::age))
        .map(Explorer::age)
        .orElseThrow();
