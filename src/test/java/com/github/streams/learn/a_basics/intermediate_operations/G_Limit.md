...
`limit(n)` truncates the stream to at most the first `n` elements in encounter order. It is a
short-circuiting operation, so it can stop an otherwise infinite or expensive stream early.

```java
List<Explorer> firstTwoRangers = explorers.stream()
        .filter(explorer -> explorer.guild() == Guild.RANGERS)
        .limit(2)
        .toList();
// firstTwoRangers = [Kael (RANGERS), Doran (RANGERS)]
```
