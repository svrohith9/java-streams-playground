...
`anyMatch` returns `true` as soon as one element satisfies the predicate. It short-circuits — it
stops scanning the moment it finds a match.

```java
boolean hasVeteran = explorers.stream()
        .anyMatch(explorer -> explorer.age() > 25);

System.out.println("hasVeteran = " + hasVeteran);
// hasVeteran = true   (Kael, Doran, Theron, Lyra all qualify)
```
